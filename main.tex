...
\begin{document}
	...
	\begin{abstract}
		...
		To address this, Concept Drift Adaptation methods based on Active Learning (CDA-AL) have been proposed to enhance model performance.
		However, limited research has been devoted to analyzing its security.
		This paper focuses on the robustness of CDA-AL, particularly under data poisoning attacks.
		The introduction of active learning techniques in CDA-AL makes the design of poisoning attack strategies more challenging.
		Therefore, we conduct a poisoning attack study targeting CDA-AL.
		We propose a method that manipulates the uncertainty ranking of test samples by crafting poisoned examples, leading to the misallocation of the victim model's labeling budget and preventing it from effectively learning the attack target.
		Furthermore, we propose a novel poisoned samples generation method that leverages naturally occurring concept drift samples as the foundation. 
		By incorporating problem space perturbations and uncertainty-based feature attribution, our approach enables the efficient generation of poisoned samples.
		
		PACDA's effectiveness is evaluated by using 7 experimental baselines, built by combining 7 models across 4 datasets.
		\textbf{PACDA achieves an average attack success rate of over 90\% on three concept drift datasets with shorter time spans.}  
